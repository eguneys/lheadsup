...
export function lookup_cards_fast(cards: Card[]) {
  return evaluate(cards.map(_ => ph2_encode(_)))
}

export function lookup_cards_str_fast(cards: string) {
  return lookup_cards_fast(split_cards(cards))
}